...
Poly = [(6245,34300),(6245,29085),(4785,29085),(4785,26570)\
...
'Poly = [(8000,8000),(9000,6000),(11000,8000),(9000,12000),(6000,12000)\
        ...
        (0,0),(5000,0),(8000,2000),(6000,6000),(7000,7000)]'
...
'Poly = [(0,0),(100,0),(100,10),(100,50),(90,50),(90,10),(80,10),(80,50),(70,50)\
        ...
        ,(30,50),(30,10),(20,10),(20,50),(10,50),(10,10),(0,10),(0,0)]'#Awesome)
...
'Poly = [(2000,16000),(2000,14000),(0,14000),(-5000,7000),(0,0),(2000,-2000),\
        (4000,0),(4000,4000),(8000,4000),(8000,0000),(14000,-5000),(20000,0),\
        (20000,6000),(15000,6000),(15000,10000),(20000,10000),(20000,14000),\
...
Poly = [(66, -22), (52, 4), (65, 4), (66,-1), (78, -4), (81,3), (78, 17), (89,15), (101, 20), (101, 31), (87, 35),
         ...
         (60,15), (36,15), (36, -8)]
Poly.reverse()
#the delaunay code starts
points = np.array(Poly,dtype = np.float64) #because the max value of dtype32 is less, hence I used dtype64
tri = Delaunay(points)
plt.triplot(points[:,0], points[:,1], tri.simplices, color = 'r')
plt.plot(points[:,0], points[:,1],color = 'r')
T = points[tri.simplices]
#print("The triangulations are:",T)
def circumcenter(T,Poly):
    Pcir = [];Pout = []
    for i in range(len(T)):
        ...
        cc = (ux, uy)
        Pcir.append(cc)
    for j in range(len(Pcir)):
        if not (Point(Pcir[j]).within(Polygon(Poly))):
            Pout.append(Pcir[j])
    for k in range(len(Pout)):
        if Pout[k] in Pcir:
            Pcir.remove(Pout[k])
    return Pcir
Pcir = circumcenter(T,Poly)
#print("The Pcir are:",Pcir)
P = Poly
AP = P
P.append(P[0])
Pc = Pcir
...
# Code perfect................................................#
T = list(T)
#print("Final Triangle:",T)
def plt_plot(P,Yn,Pcir,T):
    plot_lstx = list()
    plot_lsty = list()
